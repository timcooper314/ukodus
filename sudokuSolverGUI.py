...
class SudokuGUI:
    def __init__(self,master):
        ...
        self.entries =[]
        for n in range(9**2):
            self.entries.append(Entry(master,width=3,fg="black"))
            self.entries[n].config({"background":"white"})
            r=n//9+(n//9)//3
            c=int(n%9)+(int(n%9)//3)
            self.entries[n].grid(row=r,column=c)

        self.example_button = Button(master,text="Example",command=self.input_example)
        ...
        self.generate_slider = Scale(master,from_=0,to=10,orient=HORIZONTAL)
        self.generate_slider.grid(row=4,column=14)

        self.difficulty_label = Label(master,text="Difficulty")
        self.difficulty_label.grid(row=5,column=14)

        self.solve_button = Button(master,text="Solve",command=self.start_solve)
        ...
        self.clear_button = Button(master,text="Clear",command=self.clear_entries)
